...
class SGN(nn.Module):
    def __init__(self, num_classes, seg, batch_size, train, bias = True):
    ...
    def forward(self, input):
        ...
        #input = einops.rearrange(input, 'B T J F -> B F T J')

        #num_joints = dim //3
        #input = input.view((bs, step, num_joints, 3))
        
        # Calcola prima differenza, noi l'abbiamo già...
...
class norm_data(nn.Module):
    def __init__(self, dim= 64):
        ...
        self.bn = nn.BatchNorm1d(dim * 19)

    def forward(self, x):
        #x = einops.rearrange(x, 'B T J F -> B F J T')
